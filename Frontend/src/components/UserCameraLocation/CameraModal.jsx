...
const CameraModal = () => {
    ...
    useEffect(() => {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                () => {
                    alert('Unable to retrieve your location. Please enable Geolocation and reload to continue....');
                }
            );
        } else {
            alert('Geolocation is not supported by this browser. We recommend using Chrome Browser.');
        }
    }, []);
    ...
    return (
        <div className="font-[Poppins]">
            ...
            <p className='text-center'>Indicate the location on the map where your camera is positioned and rotate the camera in the same direction it is pointing.</p>
            <div className="location-details">
                <h1 className="text-center text-xl font-bold">Location Details</h1>
                <div style={{ height: '200px', width: '520px', borderRadius: "10px"}} ref={mapRef} className="main_map"></div>
                <div className="btns flex justify-around items-center">
                    <button type="button" className="location_btns" onClick={getCoordinates}>
                        Set location
                    </button>
                    <button type="button" className="location_btns" onClick={rotateMarkerAnticlockwise}>
                        Rotate left
                    </button>
                    <button type="button" className="location_btns" onClick={rotateMarkerClockwise}>
                        Rotate right
                    </button>
                    <button type="button" className="location_btns" onClick={removeMarker}>
                        Remove Location
                    </button>
                </div>
            </div>
