...
const ViolenceDetection = () => {
    ...
    useEffect(() => {
        setCameraEvidence(evidence.filter((evi) => evi.crime === "Violence detected"));
    }, [evidence]);
    ...
    const sendFrameToServer = async () => {
        if (videoRef.current) {
            ...
            await fetch("http://127.0.0.1:5000/violence-detection", {
                method: "POST",
                body: formData,
            })
    ...
    return (
        ...
        >
            <div className={style.one}>
                <h1 className={style.headline}>Live Violence Detection</h1>
            </div>
